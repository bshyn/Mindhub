...
function createTable(tableVar, content){
  ...
  tdMissedVotes.appendChild(document.createTextNode(Math.round(((content.total_votes*content.votes_with_party_pct)/100))));
  trElement.appendChild(tdMissedVotes);

  let tdPercentageMissed = document.createElement('td');
  tdPercentageMissed.appendChild(document.createTextNode(content.votes_with_party_pct));
  trElement.appendChild(tdPercentageMissed);
}

for(let i=0; i<10; i++){
  createTable(document.getElementById('mostLoyal'), senators[i]);
}

for(let i = senators.length-1; i>senators.length-11; i--){
  createTable(document.getElementById('leastLoyal'), senators[i]);
}
